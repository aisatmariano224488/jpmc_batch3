...
<body class="bg-gray-100">
    ...
    <div class="admin-content p-4 sm:p-6 lg:p-8">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-gray-800">Manage Awards & Timeline</h1>
                <p class="text-sm sm:text-base text-gray-600">Add, edit, or remove awards and timeline</p>
            </div>
            <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                <button class="w-full sm:w-auto bg-primary text-white px-4 py-2 rounded-lg hover:bg-secondary transition duration-300" onclick="openAddModal('award')">
                    <i class="fas fa-plus mr-2"></i> Add New Award
                </button>
                <button class="w-full sm:w-auto bg-primary text-white px-4 py-2 rounded-lg hover:bg-secondary transition duration-300" onclick="openAddModal('timeline')">
        ...
        <div class="mb-6">
            <div class="border-b border-gray-200">
                <nav class="-mb-px flex space-x-4 sm:space-x-8 overflow-x-auto">
                    <button onclick="showTab('awards')" class="tab-button border-primary text-primary whitespace-nowrap py-4 px-1 border-b-2 font-medium text-xs sm:text-sm">
                        Awards
                    </button>
                    <button onclick="showTab('timeline')" class="tab-button border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-xs sm:text-sm">
                        Timeline
                    </button>
                </nav>
            </div>
        ...
        <!-- Awards Section -->
        <div id="awards-section" class="bg-white rounded-lg shadow-md p-4 sm:p-6">
            <!-- Mobile Card View -->
            <div class="block lg:hidden">
                ...
                <div class="border-b border-gray-200 p-4 hover:bg-gray-50 last:border-b-0">
                    <div class="flex items-start gap-3 mb-3">
                        <?php if ($award['image']): ?>
                        <img src="../assets/img/awards/<?php echo htmlspecialchars($award['image']); ?>" alt="<?php echo htmlspecialchars($award['title']); ?>" class="h-16 w-16 object-cover rounded flex-shrink-0">
                        <?php else: ?>
                        <div class="h-16 w-16 bg-gray-200 rounded flex items-center justify-center flex-shrink-0">
                            <i class="fas <?php echo htmlspecialchars($award['icon']); ?> text-gray-400 text-xl"></i>
                        </div>
                        <?php endif; ?>
                        
                        <div class="flex-1 min-w-0">
                            <div class="text-sm font-semibold text-gray-900 mb-1"><?php echo htmlspecialchars($award['title']); ?></div>
                            <div class="text-xs text-gray-600 mb-2"><?php echo htmlspecialchars($award['description']); ?></div>
                            <div class="text-xs text-gray-500">Year: <?php echo htmlspecialchars($award['year']); ?></div>
                        </div>
                    </div>
                    
                    <div class="flex gap-2">
                        <button class="flex-1 text-center bg-blue-500 hover:bg-blue-700 text-white py-2 px-3 rounded text-sm" onclick="openEditModal('award', <?php echo htmlspecialchars(json_encode($award)); ?>)">
                            <i class="fas fa-edit"></i> Edit
                        </button>
                        <button class="flex-1 text-center bg-red-500 hover:bg-red-700 text-white py-2 px-3 rounded text-sm" onclick="deleteItem('award', <?php echo $award['id']; ?>)">
                ...
                <div class="py-8 text-center text-gray-500">
                    <p class="text-sm">No awards found. Click "Add New Award" to create one.</p>
                </div>
                <?php endif; ?>
            </div>
        ...
        <!-- Timeline Section -->
        <div id="timeline-section" class="bg-white rounded-lg shadow-md p-4 sm:p-6 hidden">
            <!-- Mobile Card View -->
            <div class="block lg:hidden">
                <?php foreach ($timeline as $item): ?>
                <div class="border-b border-gray-200 p-4 hover:bg-gray-50 last:border-b-0">
                    <div class="flex items-start gap-3 mb-3">
                        <div class="h-12 w-12 bg-gray-200 rounded-full flex items-center justify-center flex-shrink-0">
                            <i class="fas <?php echo htmlspecialchars($item['icon']); ?> text-gray-400"></i>
                        </div>
                        
                        <div class="flex-1 min-w-0">
                            <div class="text-sm font-semibold text-gray-900 mb-1"><?php echo htmlspecialchars($item['title']); ?></div>
                            <div class="text-xs text-gray-600 mb-2"><?php echo htmlspecialchars($item['description']); ?></div>
                            <div class="text-xs text-gray-500">Date: <?php echo date('F Y', strtotime($item['date'])); ?></div>
                        </div>
                    </div>
                    
                    <div class="flex gap-2">
                        <button class="flex-1 text-center bg-blue-500 hover:bg-blue-700 text-white py-2 px-3 rounded text-sm" onclick="openEditModal('timeline', <?php echo htmlspecialchars(json_encode($item)); ?>)">
                            <i class="fas fa-edit"></i> Edit
                        </button>
                        <button class="flex-1 text-center bg-red-500 hover:bg-red-700 text-white py-2 px-3 rounded text-sm" onclick="deleteItem('timeline', <?php echo $item['id']; ?>)">
                            <i class="fas fa-trash"></i> Delete
                        </button>
                    </div>
                </div>
                ...
                <div class="py-8 text-center text-gray-500">
                    <p class="text-sm">No timeline items found. Click "Add Timeline Item" to create one.</p>
                </div>
                <?php endif; ?>
            </div>
