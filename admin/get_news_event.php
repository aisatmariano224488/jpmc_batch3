...
// --- API: Return JSON for AJAX requests ---
if (isset($_GET['id'])) {
    header('Content-Type: application/json');
    $id = intval($_GET['id']);

    // If sections=true, return content sections
    if (isset($_GET['sections']) && $_GET['sections'] === 'true') {
        $sections = $contentSections->getSections($id);
        // For each section, add images and videos
        foreach ($sections as &$section) {
            $section['images'] = $contentSections->getSectionImages($section['id']);
            $section['videos'] = $contentSections->getSectionVideos($section['id']);
        }
        echo json_encode($sections);
        exit;
    }

    // Otherwise, return the main news/event item
